...
import { getShopConfig, listExports, getExportStats } from "../services/storage.server";
...
export default function Index() {
  ...
  const nextRun = config.syncEnabled && config.syncSchedule === 'nightly'
    ...
    <s-page heading="Sage 50 Journal Entry Sync">
      ...
      <s-section heading="Sync Status">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px' }}>
          ...
          <StatCard
            label="Last Export"
            value={recentExports.length > 0 ? format(new Date(recentExports[0].created), 'MMM d, yyyy') : 'Never'}
            status="info"
          />
        </div>
      </s-section>
      ...
      <s-section heading="Getting Started">
        <s-paragraph>
          This app generates CSV-formatted journal entries from your Shopify transactions for import into Sage 50 accounting software.
        </s-paragraph>
        ...
        <s-stack direction="block" gap="base">
          <div>
            ...
            <ol style={{ marginTop: '8px', paddingLeft: '20px' }}>
              <li><strong>Payout-First Reconciliation:</strong> Starts with money that hit your bank (payouts), then works backwards through transactions</li>
              <li><strong>Perfect Balance:</strong> All journal entries reconcile to exact payout amounts</li>
              <li><strong>CSV Export:</strong> Download Sage 50-compatible CSV files</li>
      ...
      <s-section heading="Recent Exports">
        {recentExports.length === 0 ? (
          <s-paragraph>
            No exports yet. <Link to="/app/exports" style={{ color: '#008060', textDecoration: 'underline' }}>Generate your first export</Link> to get started.
          </s-paragraph>
        ) : (
            ...
            <tbody>
              {recentExports.map((exp, index) => (
                <tr
                  key={exp.filename}
                  style={{
                    borderBottom: index < recentExports.length - 1 ? '1px solid #f6f6f7' : 'none',
                  }}
                >
                  ...
                  <td style={{ padding: '12px', textAlign: 'right', color: '#637381' }}>
      ...
      <s-section slot="aside" heading="Need Help?">
        <s-paragraph>
          <strong>CSV Format:</strong> Sage 50 compatible<br/>
          Date, Reference, Account, Debit, Credit, Memo
        </s-paragraph>
        <s-paragraph>
          <strong>Reconciliation:</strong> Payout-first approach ensures perfect balance to bank deposits
        </s-paragraph>
        <s-paragraph>
          Questions? Check the{" "}
          <s-link href="https://github.com/anthropics/claude-code/issues" target="_blank">
            documentation
          </s-link>
        </s-paragraph>
      </s-section>
...
function StatCard({ label, value, status }: StatCardProps) {
  ...
  return (
    ...
    >
      <div style={{ fontSize: '12px', color: textColors[status], marginBottom: '4px' }}>
        {label}
      </div>
      <div style={{ fontSize: '20px', fontWeight: '600', color: textColors[status] }}>
        {value}
      </div>
    </div>
  );
